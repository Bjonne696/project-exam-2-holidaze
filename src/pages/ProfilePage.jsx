...
const ProfilePage = () => {
  ...
  const buttonStyle = "border border-#810f0f rounded-full text-center px-4 py-2 bg-card-background";
  ...
  return (
    <div className="container mx-auto p-4">
     <div className="flex flex-col sm:flex-row justify-between items-center">
      ...
      {!user?.venueManager ? (
        <button onClick={handleBecomeVenueManagerClick} className={`${buttonStyle} btn-primary my-4`}>Become Venue Manager</button>
      ) : (
        <button onClick={handleRevokeManagerStatusClick} className={`${buttonStyle} btn-danger my-4`}>Revoke Manager Status</button>
      )}
        ...
        {bookings && bookings.length > 0 ? (
          bookings.map((booking) => (
            <div key={booking.id} className="p-4 border rounded shadow-sm">
              ...
              <button onClick={() => handleDeleteBooking(booking.id)} className={`btn ${buttonStyle} bg-red-500`}>Delete Booking</button>
            </div>
          ))
        ) : (
